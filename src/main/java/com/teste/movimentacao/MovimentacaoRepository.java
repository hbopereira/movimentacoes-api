...
public interface MovimentacaoRepository extends BaseRepository<Movimentacao> {
	...
	@Query(value="select m.cod, cl.nome, c.categoria, c.status, c.tipo, "
			...
			+ " (select COUNT(*) from conteiner co where co.categoria = 'Importacao') as totalImportacao, "
			+ " (select COUNT(*) from conteiner co where co.categoria = 'Exportacao') as totalExportacao "
			+ "from movimentacao m "
